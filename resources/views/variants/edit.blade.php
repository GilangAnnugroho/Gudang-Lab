...
<div class="container-fluid">
    ...
    <div class="page-title-wrap">
        <div>
            <h1 class="page-title">Edit Item Variant</h1>
            <p class="page-subtitle mb-0">
                {{ $variant->itemMaster->item_code }} — {{ $variant->itemMaster->item_name }}<br>
                <span class="text-muted">
                    Merek: <strong>{{ $variant->brand }}</strong>
                    @if($variant->lot_number)
                        · Lot: <strong>{{ $variant->lot_number }}</strong>
                    @endif
                    @if($variant->expiration_date)
                        · Exp: {{ $variant->expiration_date->format('d-m-Y') }}
                    @endif
                </span>
            </p>
        </div>
        ...
        <form action="{{ route('variants.update', $variant) }}" method="post">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    <select
                        ...
                        @foreach($items as $id => $label)
                            <option value="{{ $id }}"
                                {{ (string)old('item_master_id', $variant->item_master_id) === (string)$id ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    <div class="form-text">Ubah jika varian ini pindah ke item master lain.</div>
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('brand', $variant->brand) }}"
                           placeholder="misal: Nesco Lab, BioChem, OneMed">
                    <div class="form-text">Nama merek sesuai kemasan barang.</div>
                    @error('brand')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('lot_number', $variant->lot_number) }}"
                           placeholder="misal: REG-GLU-01, ATK-2025-A1">
                    <div class="form-text">
                        Wajib diisi untuk reagen; boleh kosong untuk ATK / RT.
                    </div>
                    @error('lot_number')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 col-lg-3">
                    ...
                    <input type="date"
                           ...
                           value="{{ old('expiration_date', optional($variant->expiration_date)->format('Y-m-d')) }}">
                    <div class="form-text">
                        Wajib diisi untuk reagen; kosongkan jika barang tidak memiliki expired.
                    </div>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center mt-4 gap-2">
                ...
                <button type="submit" class="btn btn-primary-gradient">
                    <i class="ri-save-3-line me-1"></i> Update Variant
                </button>
            </div>
        </form>
    </div>
